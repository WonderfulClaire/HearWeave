...
A reproducible synthetic benchmark compares **delay-and-sum** against **MVDR** on a smart-glasses scene with a target at 35°, an equal-power directional interferer at −60°, and isotropic channel noise at a swept input SNR.

- Full methodology, the results table, and the curve: [`docs/beambench.md`](docs/beambench.md).
- Reproducible figures (regenerated by the script):

  ![BeamBench curve](docs/assets/beambench_curve.png)
...
- [x] BeamBench integration examples (v0.3, reproducible DAS vs MVDR on synthetic glasses scene with interferer)
- [x] PyPI package and Trusted Publishing (v0.3)
...
This section tracks publications, technical reports, and notes that build on
or cite HearWeave. Submit additions via a pull request that links the
artifact and adds a one-line description.

| Type | Title | Venue / year | Link |
|---|---|---|---|
| Technical report | BeamBench: a reproducible DAS vs MVDR benchmark on synthetic smart-glasses scenes | v0.3, 2026 | [`docs/beambench.md`](docs/beambench.md) |
| Documentation | Algorithms, parameter cheat sheet, complexity, and failure modes | v0.2, 2026 | [`docs/ALGORITHMS.md`](docs/ALGORITHMS.md) |
| Documentation | Step-by-step end-to-end tutorial (7 stages) | v0.2, 2026 | [`docs/TUTORIAL.md`](docs/TUTORIAL.md) |
| Documentation | Real-device recording manifest and evaluation protocol | v0.3, 2026 | [`docs/REAL_RECORDINGS.md`](docs/REAL_RECORDINGS.md) |

_To add a paper or report: open a PR that appends a row with the venue, year,
and a stable link (DOI, arXiv ID, or repository path)._
